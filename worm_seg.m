...
%     tolerence = 0.1;
    ...
    %     [~,MaxBackground] = AmendCenterI(img,image_height,image_width,[243,294]);
    ...
    %     desired_worm_region = GetDesireImageRegion(pos, [desired_height,desired_width],[image_height,image_width]);
        ...
        if abs(Init_Worm_Area - Worm_Area) > Init_Worm_Area*Frame_Skip_Thres
            ...
            while i>Start_Index && Worm_Area_temp < Init_Worm_Area*(1+tolerence) && Worm_Area_temp > Init_Worm_Area*(1-tolerence/2) && Worm_Thres_min>0
                Worm_Thres_min = Worm_Thres_min - step;
                [~, Worm_Area_temp, ~, ~] = worm_seg_single(img, Worm_Thres_min, worm_region, Worm_Area);
            end
            Worm_Area_temp = Worm_Area;
            while i>Start_Index && Worm_Area_temp > Init_Worm_Area*(1-tolerence/2) && Worm_Area_temp < Init_Worm_Area*(1+tolerence) && Worm_Thres_max>0
                Worm_Thres_max = Worm_Thres_max + step;
                [~, Worm_Area_temp, ~, ~] = worm_seg_single(img, Worm_Thres_max, worm_region, Worm_Area);
            end
            Worm_Thres = (Worm_Thres_max+Worm_Thres_min)/2;
    %         Worm_Thres = floor(2/(1/Worm_Thres_max+1/Worm_Thres_min));
            [binary_worm_region, Worm_Area, pos, new_worm_region] = worm_seg_single(img, Worm_Thres, worm_region, Worm_Area);
            disp(['Processed image ' num2str(i) '  Worm_Thres = ' num2str(Worm_Thres) '  Worm_Area = ' num2str(Worm_Area)  ' (' num2str(i-Start_Index+1) '/' num2str(End_Index-Start_Index+1) ')']);
            desired_worm_region = GetDesireImageRegion(pos, [desired_height,desired_width],[image_height,image_width]);
        end
